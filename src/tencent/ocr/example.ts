...
const auditLogger = winston.createLogger({
  level: 'audit',
  format: combine(timestamp(), auditFormat),
  transports: [new winston.transports.Console()],
});
...
function printOCRResult(response: GeneralBasicOCRResponse, isPdf: boolean = false) {
  ...
  response.TextDetections.forEach((text, index) => {
    ...
    // 显示旋转纠正后的坐标
    logger.info(
      `  纠正后坐标: 左上角(${text.ItemPolygon.X}, ${text.ItemPolygon.Y}), 宽度:${text.ItemPolygon.Width}, 高度:${text.ItemPolygon.Height}`
    );
...
async function exampleWithUrl() {
  ...
  try {
    const request: GeneralBasicOCRRequest = {
      ImageUrl:
        'https://ocr-demo-1254418846.cos.ap-guangzhou.myqcloud.com/general/GeneralBasicOCR/GeneralBasicOCR1.jpg', // 替换为实际的图片URL
...
runExamples().catch(error => {
  ...
  auditLogger.log({
    ...
    action: 'RUN_EXAMPLES',
    result: 'FAILURE',
    message: `Error during examples execution: ${
      error instanceof Error ? error.message : 'Unknown error'
    }`,
  });
