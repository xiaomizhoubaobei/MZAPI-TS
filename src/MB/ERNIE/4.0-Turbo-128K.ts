...
/**
 * Ernie4_40_Turbo_128k类用于与百度文心大模型ERNIE-4.0-Turbo-8K-Preview进行交互
 *
 ...
 * ```typescript
 * // 创建实例
 * const ernie = new Ernie4_40_Turbo_128k('your_api_key', 'your_secret_key');
 *
 * // 基本对话示例
 ...
 * ```
 */
export class Ernie4_40_Turbo_128k extends BaiduERNIEBase {
    /**
     * 创建Ernie4_8K_Turbo_128k实例
     *
     ...
     * // 错误处理示例
     * try {
     *   const ernie = new Ernie_40_Turbo_128k('', ''); // 将抛出错误
     * } catch (error) {
    ...
    constructor(apiKey: string, secretKey: string) {
        const apiUrl = BaiduERNIEBase.BASE_URL + 'ernie-4.0-turbo-128k';
        super(apiKey, secretKey, apiUrl);
    }

    /**
     * 向百度文心大模型Ernie4_8K_Turbo_128k发送对话请求
     *
    ...
    /**
     * 向百度文心大模型Ernie4_4.0_Turbo_128k发送流式对话请求
     *
     * @param {Message[]} messages - 对话消息数组，每条消息必须包含role和content字段
