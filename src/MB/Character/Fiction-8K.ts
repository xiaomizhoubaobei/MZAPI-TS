...
/**
 * ERNIE_Character_Fiction_8K类用于与百度文心大模型ERNIE_Character_Fiction_8K进行交互
 *
 * 该类继承自BaiduERNIEBase，提供了与百度文心大模型ERNIE_Character_Fiction_8K版本API交互的功能。
 * 主要功能包括：
 * - 自动获取和管理访问令牌
 * - 发送对话请求并处理响应
 * - 参数验证和错误处理
 ...
 * ```typescript
 * // 创建实例
 * const ernie = new ERNIE_Character_Fiction_8K('your_api_key', 'your_secret_key');
 *
 * // 基本对话示例
 ...
 * ```
 */
export class ERNIE_Character_Fiction_8K extends BaiduERNIEBase {
    /**
     * 创建Ernie4_8K实例
     *
     ...
     * // 错误处理示例
     * try {
     *   const ernie = new ERNIE_Character_Fiction_8K('', ''); // 将抛出错误
     * } catch (error) {
    ...
    constructor(apiKey: string, secretKey: string) {
        const apiUrl = BaiduERNIEBase.BASE_URL + 'ernie-novel-8k';
    ...
    /**
     * 向百度文心大模型ERNIE_Character_Fiction_8K发送流式对话请求
     *
     * @param {Message[]} messages - 对话消息数组，每条消息必须包含role和content字段
