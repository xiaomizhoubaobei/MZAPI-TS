...
/**
 * ERNIE_Lite_pro_128k类用于与百度文心大模型ERNIE_Lite_pro_128k进行交互
 *
 * 该类继承自BaiduERNIEBase，提供了与百度文心大模型ERNIE_Lite_pro_128k版本API交互的功能。
 * 主要功能包括：
 * - 自动获取和管理访问令牌
 * - 发送对话请求并处理响应
 * - 参数验证和错误处理
 * - 支持自定义温度、采样参数等高级设置
 * - 支持长文本输入和输出（最大支持128K tokens）
 *
 * @example
 * ```typescript
 * // 创建实例
 * const ernie = new ERNIE_Lite_pro_128k('your_api_key', 'your_secret_key');
 *
 * // 基本对话示例
 ...
 * ```
 */
export class ERNIE_Lite_pro_128k extends BaiduERNIEBase {
    /**
     * 创建ERNIE_Lite_pro_128k实例
     *
     ...
     * // 错误处理示例
     * try {
     *   const ernie = new ERNIE_Lite_pro_128k('', ''); // 将抛出错误
     * } catch (error) {
    ...
    constructor(apiKey: string, secretKey: string) {
        const apiUrl = BaiduERNIEBase.BASE_URL + 'ernie-3.5-8k-preview';
        super(apiKey, secretKey, apiUrl);
    }

    /**
     * 向百度文心大模型ERNIE_Lite_pro_128k发送对话请求
     *
    ...
    /**
     * 向百度文心大模型ERNIE_Lite_pro_128k发送流式对话请求
     *
     * @param {Message[]} messages - 对话消息数组，每条消息必须包含role和content字段
